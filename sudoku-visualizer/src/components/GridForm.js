...
export default function GridForm(props) {
    ...
    for (let i = 0; i<81; i++) {
        ...
        inputArray.push(<input id={generateId} onChange={props.handleDigitInput} maxLength="1" className="bg-slate-100 text-center border-0 text-xl w-full h-full focus:outline-0" type='text'></input>)
    }
    ...
    return(
        <>
        <button className="absolute left-1/4 rounded-md px-6 py-1 m-0 bg-yellow-200" onClick={props.gridSwitch}>Back</button> 
